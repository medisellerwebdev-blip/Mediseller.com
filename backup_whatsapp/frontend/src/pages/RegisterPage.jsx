...
import { User, Mail, Lock, Loader2, ArrowRight, LogIn, AlertCircle } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success('Account created successfully! Welcome to Mediseller.');
        navigate('/dashboard');
      } else {
    ...
    <div className="min-h-[80vh] flex items-center justify-center py-12 px-4 bg-slate-50 relative overflow-hidden">
      ...
      <Card className="w-full max-w-md relative z-10 border-0 shadow-2xl shadow-slate-200/60 rounded-2xl overflow-hidden bg-white/80 backdrop-blur-sm border-t border-white/20">
        ...
        <CardHeader className="text-center pb-2 pt-8">
          ...
          <CardTitle className="font-heading text-3xl font-bold text-slate-900">Create Account</CardTitle>
          <CardDescription className="text-slate-500 text-lg">
            Join thousands of patients saving on medications.
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="pt-6">
          ...
          <div className="relative my-8">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-white px-4 text-slate-500 font-medium">Or join with</span>
            </div>
          </div>
        ...
        <CardFooter className="flex flex-col space-y-4 pb-8 pt-2">
          ...
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex gap-3 items-start">
            ...
            <p className="text-xs text-slate-600 leading-relaxed">
              By creating an account, you agree to our <Link to="/legal/terms" className="text-primary hover:underline">Terms of Service</Link> and <Link to="/legal/privacy" className="text-primary hover:underline">Privacy Policy</Link>.
            </p>
          </div>
        </CardFooter>
      </Card>
